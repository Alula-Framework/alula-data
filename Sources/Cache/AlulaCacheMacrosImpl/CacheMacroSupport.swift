...
enum CacheMacroSupport {
    ...
    static func parseArguments(
        ...
        for argument in list {
            ...
            case "namespace":
                ...
                // config key, and Alula's env-var override layer maps keys
                // through `ALULA_` + uppercase + `.` → `_` — so a namespace
    ...
    /// `[AlulaCache.CacheKey.part(a), AlulaCache.CacheKey.part(b)]` — the
    /// generic constraint on `part(_:)` is the conformance check.
    static func partsLiteral(for target: Target) -> String {
        "[" + target.keyParameterNames.map { "AlulaCache.CacheKey.part(\($0))" }.joined(separator: ", ") + "]"
    }
