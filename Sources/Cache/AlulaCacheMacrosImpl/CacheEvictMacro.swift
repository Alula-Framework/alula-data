...
public struct CacheEvictMacro: BodyMacro {
    public static func expansion(
        ...
        let evict: ExprSyntax = """
            await AlulaCache.AlulaCaches.current.evict(namespace: \(raw: namespace), parts: \(raw: parts))
            """
        ...
        if target.returnType == "Void" {
            ...
            let run: DeclSyntax = """
                let _alulaCacheResult: \(raw: target.returnType) = \(raw: effect) { () async\(raw: closureThrows) -> \(raw: target.returnType) in
                    \(raw: target.bodyText)
                }()
                """
            let giveBack: StmtSyntax = "return _alulaCacheResult"
            return [
