...
/// Alula polling for it.
///
/// Unlike Alula Data Valkey's pool, this adapter holds a `ValkeyClient` —
...
/// caught, logged, counted (`alula.cache.store_errors`), and answered with
/// a miss/no-op.
///
...
public final class ValkeyCache: Cache, Sendable {
    /// Every stored key: `alula-cache:` + `CacheKey.storageKey` — one
    /// recognizable, greppable prefix in a store that may hold non-cache
    /// data too.
    public static let keyPrefix = "alula-cache:"
    ...
    public init(
        ...
        let logger = logger ?? Logger(label: "alula.cache.valkey")
...
extension Duration {
    ...
    /// TTL never becomes "delete now" — the same rule as Alula Data Valkey's
    /// delta V3.
    ///
