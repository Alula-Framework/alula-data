...
The shared cache backend: `AlulaCache` over Valkey.
...
Every key is stored under a fixed `alula-cache:` prefix followed by
`CacheKey.storageKey` — so `alula-cache:prices:123:eu` is greppable in a
