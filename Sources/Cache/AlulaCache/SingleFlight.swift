...
public actor SingleFlight {
    ...
    /// Joins the alula for `key`. The first caller becomes the leader and
    ...
    /// Resolves the alula for `key`, resuming every parked waiter.
    public func complete(_ key: CacheKey, with outcome: Outcome) {
        ...
        // Nothing more can join this alula, so anyone awaiting a
    ...
    /// Callers currently coalesced onto `key`'s alula, excluding its
    /// leader.
