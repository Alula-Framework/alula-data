...
public final class CacheRuntime: Sendable {
    ...
    public init(
        ...
        logger: Logger = Logger(label: "alula.cache")
    ) throws {
        self.store = store
        self.codec = codec
        self.logger = logger
        ...
        while true {
            ...
            case .lead:
                do {
                    ...
                    // this caller's miss and its alula entry, a previous
                    // alula may have completed and populated the entry.
    ...
    /// alula layer, and without a non-Optional `Value` ever pretending it can.
    ...
    private struct NamespaceMetrics {
        ...
        init(namespace: String) {
            ...
            hits = Counter(label: "alula.cache.hits", dimensions: dimensions)
            misses = Counter(label: "alula.cache.misses", dimensions: dimensions)
            puts = Counter(label: "alula.cache.puts", dimensions: dimensions)
            evictions = Counter(label: "alula.cache.evictions", dimensions: dimensions)
            coalesced = Counter(label: "alula.cache.coalesced", dimensions: dimensions)
        }
    }
