...
/// The seam the expansions target: Alula Core has no ambient container and
...
/// (`AlulaCache.AlulaCaches.current`) so it resolves in any client module.
///
/// Core used to solve the same problem the same way, with `AlulaTransactions`
...
///    other cache failure.
public enum AlulaCaches {
    /// Task-local override — tests bind it with
    /// `AlulaCaches.$override.withValue(runtime) { ... }`.
    ...
    /// Installs the process-wide runtime. `AlulaCacheModule` calls this
    ...
    private static func warnUnwiredOnce() {
        ...
        if shouldWarn {
            Logger(label: "alula.cache").warning(
                "a cache annotation ran but no cache runtime is installed — every call computes. Add AlulaCacheModule to bootstrap's modules to enable caching."
            )
        }
    }
}
