...
public final class PostgresDataSource: DataSource, Sendable {
    ...
    private init(
        ...
        self.logger = logger ?? Logger(label: "alula.data.postgres.\(name)")
    ...
    ///
    /// Correct shutdown ordering is what should prevent this (`AlulaCore`'s
    ...
    /// directly, an application whose services do not go through Alula's
    ...
    /// spin. Same numbers as Alula Data Valkey's pool, deliberately — the
