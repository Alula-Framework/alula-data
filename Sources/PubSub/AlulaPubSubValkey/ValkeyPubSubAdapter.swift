...
///
/// `AlulaPubSub` handles delivery *within* a process on its own; this is the
/// hop *between* processes. Handing one to `AlulaPubSubModule` turns three
...
/// ```
///
/// `AlulaPubSubValkeyModule` builds one from `pubsub.valkey.url` and exposes
...
public struct ValkeyPubSubAdapter: DistributedPubSubAdapter {
    ...
    ///     `AlulaPubSubValkeyModule` puts it in the application's service
    ///     group.
    ///   - channel: The Valkey channel every node publishes to and subscribes
    ///     to. One channel carries every topic: Alula's own `Message` names
    ...
    public init(
        ...
        logger: Logger = Logger(label: "alula.pubsub.valkey")
    ) {
        self.client = client
...
/// verbatim.
///
/// `Message` is `Codable`, but encoding it directly would make Alula's
...
enum WireMessage {
    ...
    static func decode(_ frame: Data) throws -> Message {
        ...
        guard frame.count >= prefix, Array(frame.prefix(magic.count)) == magic else {
            throw WireMessageError.notAAlulaFrame
        }
...
enum WireMessageError: Error, Equatable, CustomStringConvertible {
    ...
    var description: String {
        switch self {
        case .notAAlulaFrame:
            return "not an Alula PubSub frame — something else is publishing to this channel, or a node is running an incompatible build."
        case .truncated:
