...
Carries Alula's PubSub between nodes over Valkey, which is what turns three
...
Override `pubsub.valkey.channel` to run two independent Alula clusters
against one Valkey.
...
``AlulaPubSubValkeyModule``'s service stops the relay *before* the client
