...
/// because this target does not depend on AlulaCacheValkey; they are kept the
...
public struct ValkeyPubSubSettings: Sendable, Equatable {
    ...
    public static let defaultChannel = "alula-pubsub"
    ...
    ///
    /// One channel carries every topic: an Alula `Message` names its own
    /// topic, and a channel per topic would mean re-subscribing every time a
    /// socket joined a room. Override it to run two independent Alula
    ...
    /// adapter's and Alula Data Valkey's:
    ///
...
public enum ValkeyPubSubConfigurationError: Error, Equatable, CustomStringConvertible {
    ...
    public var description: String {
        ...
        case .missingURL:
            return """
                AlulaPubSubValkeyModule needs \(ValkeyPubSubConfigKey.url). Set it in alula.yaml \
                or as ALULA_PUBSUB_VALKEY_URL, e.g. valkey://localhost:6379.
                """
