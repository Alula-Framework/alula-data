...
///
/// The checksum is captured at **build time** by `alula-migrate-gen` from the migration's
...
public enum MigrationChecksum {
    ...
    ///
    /// It still spells the project's name before it became Alula, and must: every
    /// checksum already recorded in a database was computed with it, so changing it
    /// would report every applied migration as drifted (D45).
