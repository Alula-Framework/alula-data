...
public struct AlulaMigrator: Sendable {
    ...
    ///
    /// The bytes spell the project's name before it became Alula, and they stay: a
    /// deploy running this version and one running a pre-rename version must
    /// contend on the same lock, or both could migrate at once (D45).
    public static let defaultAdvisoryLockKey = Int64(0x464C_4947_4854_4D47)

    /// The default bookkeeping table.
    public static let defaultMigrationsTable = "alula_migrations"

    /// The default bookkeeping table before the project was renamed from Flight.
    ///
    /// A database migrated by a pre-rename version has its ledger here. When the
    /// configured table is the default and only this one exists, a locked run renames
    ...
    public struct Configuration: Sendable {
        ...
        public init(
            ...
            logger: Logger = Logger(label: "alula-migrate"),
    ...
    public func planMigrate() async throws -> MigrationPlan {
        ...
        return try await database.withSession { session in
            let applied = try await fetchAppliedIfLedgerExists(session, adopting: false)
            try verifyIntegrity(applied: applied, entries: entries)
            let appliedVersions = Set(applied.map(\.version))
    ...
    public func repair() async throws -> RepairOutcome {
        ...
        let byVersion = Dictionary(uniqueKeysWithValues: entries.map { ($0.version, $0) })
        return try await withLockedSession { session in
            guard try await adoptLedger(session) else {
    ...
    /// The applied rows, or none when there is no ledger.
    ///
    /// A locked caller (`adopting`) goes on to write the configured table, so a legacy
    /// ledger is renamed first; an unlocked, read-only caller reads it where it is.
    private func fetchAppliedIfLedgerExists(
        _ session: any MigrationSession, adopting: Bool
    ) async throws -> [AppliedMigrationRecord] {
        if adopting {
            guard try await adoptLedger(session) else { return [] }
            return try await session.fetchApplied(configuration.migrationsTable)
        }
        guard let table = try await locateLedger(session) else { return [] }
        return try await session.fetchApplied(table)
    }

    /// The table holding the ledger: the configured one, or — only when the
    /// configured one is the default and does not exist — the pre-rename default.
    private func locateLedger(_ session: any MigrationSession) async throws -> String? {
        if try await session.migrationsTableExists(configuration.migrationsTable) {
            return configuration.migrationsTable
        }
        guard configuration.migrationsTable == Self.defaultMigrationsTable,
            try await session.migrationsTableExists(Self.legacyMigrationsTable)
        else {
            return nil
        }
        return Self.legacyMigrationsTable
    }

    /// Whether the configured ledger exists, renaming a pre-rename one into place
    /// first. Callers hold the advisory lock, so a pre-rename deploy — which takes the
    /// same lock — cannot be using the legacy table while it moves.
    private func adoptLedger(_ session: any MigrationSession) async throws -> Bool {
        guard let table = try await locateLedger(session) else { return false }
        guard table != configuration.migrationsTable else { return true }
        configuration.logger.notice(
            "adopting pre-rename bookkeeping table",
            metadata: ["from": "\(table)", "to": "\(configuration.migrationsTable)"])
        try await session.execute(
            "ALTER TABLE \(SQL.identifier(table)) RENAME TO \(SQL.identifier(configuration.migrationsTable))"
        )
        return true
    }
    ...
    private func performRollback(
        ...
        let byVersion = Dictionary(uniqueKeysWithValues: entries.map { ($0.version, $0) })
        return try await withLockedSession { session in
            let applied = try await fetchAppliedIfLedgerExists(session, adopting: true)
