...
#
# Set ALULA_KEEP_SERVERS=1 to leave the containers running between runs.
#
set -euo pipefail
cd "$(dirname "$0")/.."
...
pg_outage_name="alula-postgres-outage"
vk_outage_name="alula-valkey-outage"
...
start "$pg_name" postgres:16-alpine "$pg_port:5432" \
  -e POSTGRES_PASSWORD=alula -e POSTGRES_DB=alula_test
...
export ALULA_POSTGRES_TEST_DATABASE_URL="postgres://postgres:alula@127.0.0.1:$pg_port/alula_test" ALULA_MIGRATE_TEST_DATABASE_URL="postgres://postgres:alula@127.0.0.1:$pg_port/alula_test" ALULA_VALKEY_TEST_URL="redis://127.0.0.1:$vk_port"
export ALULA_POSTGRES_OUTAGE_TEST=1 ALULA_VALKEY_OUTAGE_TEST=1
