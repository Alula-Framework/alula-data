...
actor FakeDatabase: MigrationDatabase, MigrationSession {
    ...
    /// A pre-rename ledger in `flight_migrations`, holding `rows` until renamed.
    private(set) var legacyTable: Bool
    private var heldLocks: Set<Int64> = []

    private var failures: [(matches: @Sendable (Op) -> Bool, error: any Error, remaining: Int)] = []

    init(
        tableCreated: Bool = false, legacyTable: Bool = false,
        seededRows: [AppliedMigrationRecord] = []
    ) {
    ...
    func execute(_ sql: String) async throws {
        ...
        if sql.hasPrefix("ALTER TABLE \"\(AlulaMigrator.legacyMigrationsTable)\" RENAME TO") {
            guard legacyTable, !tableCreated else {
                throw StateError(description: "renamed a ledger that was not there, or onto one")
            }
            legacyTable = false
            tableCreated = true
        }
    ...
    func migrationsTableExists(_ table: String) async throws -> Bool {
        ...
        return table == AlulaMigrator.legacyMigrationsTable ? legacyTable : tableCreated
    }
