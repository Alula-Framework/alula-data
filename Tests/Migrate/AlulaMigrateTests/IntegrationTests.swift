...
/// Integration tests against a real Postgres, gated by `ALULA_MIGRATE_TEST_DATABASE_URL`
/// (e.g. `postgres://postgres:alula@127.0.0.1:55432/alula_test?sslmode=disable`; see
...
let integrationDatabaseURL = ProcessInfo.processInfo.environment["ALULA_MIGRATE_TEST_DATABASE_URL"]
...
func testConfiguration(ledger: String) -> AlulaMigrator.Configuration {
    var logger = Logger(label: "alula-migrate-test")
    logger.logLevel = .error
    var key = Int64(5_577_000_001)
    for byte in ledger.utf8 {
        key = key &* 31 &+ Int64(byte)
    }
    return AlulaMigrator.Configuration(
...
struct IntegrationTests {
    ...
    @Test func checksumDriftHaltsAndRepairRebaselines() async throws {
        try await withTestClient { client in
            ...
            let driftedMigrator = AlulaMigrator(
    ...
    @Test func exampleBinarySmokeTest() async throws {
        ...
        let example = testBinary.deletingLastPathComponent()
            .appendingPathComponent("alula-migrate-example")
        guard FileManager.default.isExecutableFile(atPath: example.path) else {
            Issue.record("alula-migrate-example not found next to the test binary; build it first")
            return
        }
