...
@testable import AlulaPubSubValkey
...
@Suite(
    ...
    .enabled(if: PubSubTestServer.isConfigured, "set ALULA_VALKEY_TEST_URL"))
struct ValkeyPubSubAdapterTests {
    ...
    func crossesBetweenNodes() async throws {
        let channel = "alula-test-\(UUID().uuidString)"
    ...
    func channelsAreIsolated() async throws {
        // Two Alula clusters against one Valkey must not bleed into each
        // other; the channel is what separates them.
        let mine = "alula-test-\(UUID().uuidString)"
        let theirs = "alula-test-\(UUID().uuidString)"
