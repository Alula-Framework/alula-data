...
enum TestServer: String, CaseIterable, Sendable, CustomStringConvertible {
    ...
    ///
    /// These tests `flushdb`, and so do AlulaDataValkeyTests — two suites in
...
struct ValkeyCacheIntegrationTests {
    ...
    @Test("set/get round-trip, under the alula-cache: key prefix", arguments: TestServer.available)
    func roundTrip(_ server: TestServer) async throws {
        try await withCache(server) { cache in
            ...
            // the store.
            let raw = try await cache.client.get("alula-cache:prices:123:eu")
            #expect(raw.map { Data($0) } == Data("42".utf8))
            // And carries no expiry when ttl is nil.
            let pttl = try await cache.client.pttl("alula-cache:prices:123:eu")
            #expect(pttl == -1)
        }
    }
    ...
    func ttlExpiry(_ server: TestServer) async throws {
        try await withCache(server) { cache in
            ...
            let remaining = try await cache.client.pttl("alula-cache:prices:ttl")
