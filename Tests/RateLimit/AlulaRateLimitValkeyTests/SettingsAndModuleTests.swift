...
struct SettingsTests {
    ...
    @Test("the URL key is the one AlulaRateLimitModule probes for")
    func probeKeyMatchesAlula() {
        // AlulaRateLimit's `ValkeyRateLimitConfigKeyProbe.url` is the other
    ...
    func defaults() throws {
        ...
        #expect(settings.keyPrefix == "alula-rate-limit:")
    }
...
@Suite("AlulaRateLimitValkeyModule — wiring")
struct ModuleTests {
    ...
    func providesStore() throws {
        ...
        let module = try AlulaRateLimitValkeyModule(configuration: configuration)
        ...
        #expect(application.services.contains { $0.moduleName == "AlulaRateLimitValkeyModule" })
    }

    @Test("a malformed URL fails composition, never the first limited request")
    func badURLFailsComposition() {
        #expect(throws: (any Error).self) {
            _ = try AlulaRateLimitValkeyModule(
    ...
    func missingURLFailsComposition() {
        #expect(throws: (any Error).self) {
            _ = try AlulaRateLimitValkeyModule(configuration: Configuration())
        }
    }
}
